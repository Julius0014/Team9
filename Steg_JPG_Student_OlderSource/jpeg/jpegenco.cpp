...
void JpegEncoder::DoCopy (const JpegEncoder &source)
{
  ...
  image_quality = source.image_quality ; // jao

//  gImageQuality = image_quality; //jao NOT called
...
//
void JpegEncoder::WriteImage (std::ostream &strm, BitmapImage &image)
{
  bit_count = 0 ;
  current_image = &image ;
  ...
  //for(int i =0; i < 4; i++) printf("q = %d\n", image_quality);
  //CreateQuantizationTables (gImageQuality) ;//jao
...
void JpegEncoder::Initialize ()
{
  ...
  image_quality = gImageQuality ; // jao
...
void JpegEncoder::CreateQuantizationTables (unsigned int quality) // jao
{
  ...
  // Scale Factor
  double scale = exp ( (6 * (50 - (int) quality)/50.0) * log (2.0)) ; //natural log of 2 = 0.693147

  for (ii = 0 ; ii < JpegSampleSize ; ++ ii)
  {
    unsigned int value = (unsigned int) 
         (default_luminance_table [JpegZigZagInputOrder (ii)] * scale) ;

    if (value < JpegMinQuantizationValue)
      (*luminance_quanttbl) [ii] = JpegMinQuantizationValue ;		// min is 1
    else if (value > JpegMax8BitQuantizationValue)
      (*luminance_quanttbl) [ii] = JpegMax8BitQuantizationValue ; // max is 255
    else
  ...
  {
    unsigned int value = (unsigned int) 
        (default_chrominance_table [JpegZigZagInputOrder (ii)] * scale) ;
    if (value < 1)
...
{
  unsigned int length = (unsigned int) (sizeof (UBYTE2) + str.length () + 1);
  OutputMarker (COM) ;
  OutputWord (length) ;
...
void JpegEncoder::OutputJfifHeader ()
{
  ...
  jfif.xthumbnail = 0 ;
  jfif.ythumbnail = 0 ;
  output_stream->write ((char *) &jfif, sizeof (jfif)) ;
  return ;
}
...
//    The quality value.
//
unsigned int JpegEncoder::GetQuality () 
{
...
void JpegEncoder::SetQuality (unsigned int value)
{
  ...
  image_quality = value ; // jao NOT called
  return ;
}
