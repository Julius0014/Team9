...
void JpegDecoder::ReadStreamHeader ()
{
  ...
  tmpB = ReadByte ();
  if (tmpB != APP0 && tmpB != APP0 + 1)
    throw EJpegBadData ("Missing JFIF//Exif APP0 Marker") ;

  JfifHeader header ;
  input_stream->read ((char *) &header, sizeof (header)) ;
  if (memcmp ("JFIF", (char *) header.identifier, 4) != 0 && memcmp("Exif", (char *) header.identifier, 4) != 0)
